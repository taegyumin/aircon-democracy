...
// `~/.aircon-env` → `NEXT_PUBLIC_NCP_MAPS_CLIENT_ID` 로 alias 되어 있어야 함.
...
export function loadNaverMaps(): Promise<NaverMapsModule> {
  ...
  const clientId = process.env.NEXT_PUBLIC_NCP_MAPS_CLIENT_ID as string | undefined;
  if (!clientId) {
    loadPromise = Promise.reject(new Error('NEXT_PUBLIC_NCP_MAPS_CLIENT_ID is not configured'));
    return loadPromise;
  }
